...
-- Idempotent. Ensures a single seeded 'Admin' role exists and holds every
-- Action currently in the catalog. Safe to re-run on any tenant database,
-- new or existing -- re-running after 03_Actions_Seed.sql adds new modules
-- extends Admin's grants to cover them.
--
-- Must run AFTER 03_Actions_Seed.sql (grants every Action that exists at the
